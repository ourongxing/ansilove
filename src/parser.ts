...
import type { DisplayData, FileObj, ParsedData, RenderOptions, Sauce } from "./types"
import { display, displayDataToCanvas, validateOptions } from "./display"
...
function ans(bytes: Uint8Array, options: RenderOptions): ParsedData {
  ...
  function getValues(): number[] {
    return escapeCode.substr(1, escapeCode.length - 2).split(";").map((value: string): number => {
...
function pcb(bytes: Uint8Array, options: RenderOptions): ParsedData {
  ...
  const icecolors = file.sauce ? file.sauce.flags & 1 || options.icecolors || 0 : options.icecolors || 0
...
function xb(bytes: Uint8Array, options: RenderOptions): ParsedData {
  ...
  function xBinHeader(file: FileObj): XBinHeader {
    ...
    if (header.fontHeight === 0 || header.fontHeight > 32) {
      throw new Error(`Illegal value for the font height (${header.fontHeight}).`)
    }
    ...
    if (header.fontHeight !== 16 && !header.font) {
      throw new Error("A non-standard font size was defined, but no font information was included with the file.")
    }
  ...
  function uncompress(file: FileObj, width: number, height: number): Uint8Array {
  ...
  imageData.palette = header.palette ? PaletteModule.triplets16(file) : PaletteModule.BIN
  imageData.font = header.font ? FontModule.xbin(file, header.fontHeight, options) : FontModule.preset("80x25", options)
...
export function readBytes(
  ...
  if (splitRows > 0) {
    ...
    for (let start = 0; start < data.imageData.rows; start += splitRows) {
      const displayData = display(data.imageData, start, splitRows, options)
      returnArray.push(options.imagedata ? displayData : displayDataToCanvas(displayData) as unknown as DisplayData)
    }
    callback(returnArray, data.sauce)
  } else {
    const displayData = display(data.imageData, 0, data.imageData.rows, options)
    callback(options.imagedata ? displayData : displayDataToCanvas(displayData) as unknown as DisplayData, data.sauce)
  }
}
