...
import { httpGet, sauce, sauceBytes } from "./file"
...
export function render(
  ...
  httpGet(url, (bytes: Uint8Array): void => {
    options = options || {}
    if (!options.filetype) {
...
export function animate(
  ...
  return {
    ...
    load(url: string, callback: (sauce?: Sauce) => void, callbackFail?: (error: unknown) => void): void {
      httpGet(url, (bytes: Uint8Array): void => {
        if (ansimation) {
...
if (typeof self !== "undefined" && self.WorkerLocation) {
  self.onmessage = function (evt: MessageEvent): void {
    if (evt.data.bytes) {
      if (evt.data.split > 0) {
        AnsiLove.splitRenderBytes(evt.data.bytes, (imagedata: (DisplayData | HTMLCanvasElement)[], sauce?: Sauce): void => {
          self.postMessage({ splitimagedata: imagedata, sauce })
        }, evt.data.split, {
          "imagedata": 1,
          "font": evt.data.font,
          "bits": evt.data.bits,
        ...
        AnsiLove.renderBytes(evt.data.bytes, (imagedata: DisplayData | HTMLCanvasElement, sauce?: Sauce): void => {
      ...
      if (evt.data.split > 0) {
        AnsiLove.splitRender(evt.data.url, (imagedata: (DisplayData | HTMLCanvasElement)[], sauce?: Sauce): void => {
          self.postMessage({ splitimagedata: imagedata, sauce })
        }, evt.data.split, {
          "imagedata": 1,
          "font": evt.data.font,
        ...
        AnsiLove.render(evt.data.url, (imagedata: DisplayData | HTMLCanvasElement, sauce?: Sauce): void => {
          self.postMessage({ imagedata, sauce })
        }, {
          "imagedata": 1,
          "font": evt.data.font,
